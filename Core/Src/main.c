...
int16_t motor1_L_midpoint   = 2150;
int16_t motor3_R_midpoint   = 2150;

int16_t motor2_L_midpoint   = 2150;
int16_t motor4_R_midpoint   = 2150;
...
////  __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_4, 19999);
    ...
    if (elrs_data.Switch == 1)
    {
      ...
      motor_2_pid.Kp = 30;
			motor_4_pid.Kp = 30;
			const int16_t motor1_L_ready		=	motor1_L_midpoint	+	elrs_data.midpoint	+	elrs_data.midpoint_1;
			const int16_t motor3_R_ready		=	motor3_R_midpoint	+	elrs_data.midpoint	+	elrs_data.midpoint_1;

      const int16_t motor2_L_ready		=	motor2_L_midpoint	+	elrs_data.midpoint	+	elrs_data.midpoint_1;
			const int16_t motor4_R_ready		=	motor4_R_midpoint	+	elrs_data.midpoint	+	elrs_data.midpoint_1;
			...
			uint32_t tick = HAL_GetTick();
                ...
                if ((int32_t)(tick - sm_next_tick) >= 0)
                {
                    ...
                    Wings_Data.Wings_motor[0].Target_Angle	=	(int16_t)(motor1_L_ready		-	q15_mul(ampL, c));
                    Wings_Data.Wings_motor[2].Target_Angle	=	(int16_t)(motor3_R_ready		-	q15_mul(ampR, c));
                    
                    Wings_Data.Wings_motor[1].Target_Angle	=	(int16_t)(motor2_L_ready		-	q15_mul(ampL, c));
                    Wings_Data.Wings_motor[3].Target_Angle	=	(int16_t)(motor4_R_ready		-	q15_mul(ampR, c));
        ...
        else if (elrs_data.Mode == 1)
		{
			...
			motor_4_pid.Kp = 10;
			// 基准角
			const int16_t ago_targetL		=	motor1_L_midpoint	+	elrs_data.midpoint	+	elrs_data.midpoint_1	-	elrs_data.Yaw;
			const int16_t ago_targetR		=	motor3_R_midpoint	+	elrs_data.midpoint	+	elrs_data.midpoint_1	+	elrs_data.Yaw;

      const int16_t after_targetL		=	motor2_L_midpoint	+	elrs_data.midpoint	+	elrs_data.midpoint_1	-	elrs_data.Yaw;
      const int16_t after_targetR		=	motor4_R_midpoint	+	elrs_data.midpoint	+	elrs_data.midpoint_1	+	elrs_data.Yaw;
			// 从当前值缓慢移动到目标值
        ...
        if (current1 < after_targetL) {
          current1 += MIN(MAX_STEP, after_targetL - current1);
        } else if (current1 > after_targetL) {
          current1 -= MIN(MAX_STEP, current1 - after_targetL);
        }
        Wings_Data.Wings_motor[1].Target_Angle = current1;
        // 电机3（右后）平滑移动
        int16_t current3 = Wings_Data.Wings_motor[3].Corrective_Angle;
        if (current3 < after_targetR) {
          current3 += MIN(MAX_STEP, after_targetR - current3);
        } else if (current3 > after_targetR) {
          current3 -= MIN(MAX_STEP, current3 - after_targetR);
        }
      ...
      const int16_t after_targetL		=	1024;
      const int16_t after_targetR		=	1024;
      ...
      motor_2_pid.Kp=12*(float)abs16_fast(after_targetL-Wings_Data.Wings_motor[1].Corrective_Angle)/(float)abs16_fast(after_targetR-Wings_Data.Wings_motor[3].Corrective_Angle);
      motor_4_pid.Kp=12*(float)abs16_fast(after_targetR-Wings_Data.Wings_motor[3].Corrective_Angle)/(float)abs16_fast(after_targetL-Wings_Data.Wings_motor[1].Corrective_Angle);
			}
			StarAndGetResult();
			Motor_PID_Control();
